...
let imageGenerations = new Map();
let initialImagesGeneratedCount = 0;
let isInitialGeneration = false;

// --- Branding State ---
let brandingText = '';
let brandingTextColor = 'white';
let brandingLogoBase64 = null;
let brandingSize = 8;
let brandingOpacity = 0.7;
let brandingPosition = { x: 20, y: 20 }; // Position on the 1080px canvas
...
const brandingEditor = document.getElementById('branding-editor');
const brandingTextInput = document.getElementById('branding-text-input');
const brandingLogoInput = document.getElementById('branding-logo-input');
const brandingLogoLabel = document.getElementById('branding-logo-label');
const brandingLogoPreviewWrapper = document.getElementById('branding-logo-preview-wrapper');
const brandingLogoPreview = document.getElementById('branding-logo-preview');
const brandingRemoveLogoButton = document.getElementById('branding-remove-logo-button');
const brandingSizeSlider = document.getElementById('branding-size-slider');
const brandingOpacitySlider = document.getElementById('branding-opacity-slider');

// --- Output elements ---
const outputContainer = document.getElementById('results-container');
const captionContainer = document.getElementById('caption-container');
const postOptionsGrid = document.getElementById('post-options-grid');
...
const previewEditBrandingBtn = document.getElementById('preview-edit-branding-btn');

// --- Prompt Modal Elements ---
const promptModalOverlay = document.getElementById('prompt-modal-overlay');
const closePromptButton = document.getElementById('close-prompt-button');
const promptModalHeaderInput = document.getElementById('prompt-modal-header-input');
const promptModalSubheaderInput = document.getElementById('prompt-modal-subheader-input');
const promptModalImagePromptInput = document.getElementById('prompt-modal-image-prompt-input');
const regenerateWithPromptButton = document.getElementById('regenerate-with-prompt-button');

// --- Branding Editor Modal Elements ---
const brandingEditorModalOverlay = document.getElementById('branding-editor-modal-overlay');
const closeBrandingEditorButton = document.getElementById('close-branding-editor-button');
const saveBrandingPositionButton = document.getElementById('save-branding-position-button');
const brandingEditorPreviewContainer = document.getElementById('branding-editor-preview-container');
const brandingEditorPreviewImage = document.getElementById('branding-editor-preview-image');
const brandingEditorOverlayElement = document.getElementById('branding-editor-overlay-element');
...
async function generateCaption(postContent) {
...
async function generateImage(postContent, index) {
    incrementApiCount();
    updateApiCounterDisplay();
...
async function renderImageThumbnail(index) {
...
async function displayImage(base64Image, index) {
...
function displayError(message, index) {
    const container = postOptionsGrid.querySelector(`[data-index="${index}"]`);
    if (container) {
        ...
        container.querySelector('.regen-btn-small')?.addEventListener('click', (e) => handleRegenerateImage(e, index));
    }
}
...
async function handleRegenerateImage(e, index) {
    e.stopPropagation();
...
async function handleNavigateVersion(e, index, direction) {
    e.stopPropagation();
    const imageData = imageGenerations.get(index);
...
async function handleRegenerateWithEditedPrompt() {
    ...
    const container = postOptionsGrid.querySelector(`[data-index="${selectedImageIndex}"]`);
    if (container) {
        const loader = document.createElement('div');
        loader.className = 'thumbnail-loader';
        loader.innerHTML = `<div class="spinner"></div>`;
...
function handleDownloadImage(imageSrc, postContent) {
...
function initializeBrandingControls() {
    ...
    const brandingColorRadios = document.querySelectorAll('input[name="branding-color"]');
    brandingColorRadios.forEach(radio => {
        radio.addEventListener('change', () => {
            const checkedRadio = document.querySelector('input[name="branding-color"]:checked');
            if (checkedRadio) {
                brandingTextColor = checkedRadio.value;
                handleBrandingChange();
            }
        });
    });
    ...
    brandingLogoInput.addEventListener('change', (e) => {
        ...
        if (file) {
            ...
            reader.onload = (event) => {
                brandingLogoBase64 = event.target?.result;
...
async function processImage(originalSrc) {
